...
export async function router(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
	...
	try {
		...
		if (resource.toLowerCase().includes('custom api call')) {
			console.log('Tentativa de usar Custom API Call detectada no recurso. Esta operação não é suportada.');
			return [[]];
		}
		...
		if (operation === '__CUSTOM_API_CALL__') {
			console.log('Tentativa de usar Custom API Call detectada como operação. Esta operação não é suportada.');
			return [[]];
		}
		...
		if (operation.toLowerCase().includes('custom api call')) {
			console.log('Tentativa de usar Custom API Call detectada na operação. Esta operação não é suportada.');
			return [[]];
		}

	} catch (error) {
		// Se não conseguir obter os parâmetros, retorna um array vazio
		console.log('Erro ao obter parâmetros no nó Hotmart:', error);
		return [[]];
	}
		...
		switch (resource) {
			case 'subscription': {
				...
				if (typeof subscriptionOperation?.execute === 'function') {
					executionResult = await subscriptionOperation.execute.call(
						this,
						items,
					);
				} else {
			...
			case 'sales': {
				...
				if (typeof salesOperation?.execute === 'function') {
					executionResult = await salesOperation.execute.call(
						this,
						items,
					);
				} else {
			...
			case 'product': {
				...
				if (typeof productOperation?.execute === 'function') {
					executionResult = await productOperation.execute.call(
						this,
						items,
					);
				} else {
			...
			case 'coupon': {
				...
				if (typeof couponOperation?.execute === 'function') {
					executionResult = await couponOperation.execute.call(
						this,
						items,
					);
				} else {
			...
			case 'club': {
				...
				if (typeof clubOperation?.execute === 'function') {
					executionResult = await clubOperation.execute.call(
						this,
						items,
					);
				} else {
			...
			case 'tickets': {
				...
				if (typeof ticketsOperation?.execute === 'function') {
					executionResult = await ticketsOperation.execute.call(
						this,
						items,
					);
				} else {
			...
			case 'negotiate': {
				...
				if (typeof negotiateOperation?.execute === 'function') {
					executionResult = await negotiateOperation.execute.call(
						this,
						items,
					);
				} else {
