...
describe('HotmartTrigger - PURCHASE_BILLET_PRINTED', () => {
  let trigger: HotmartTrigger;
  let triggerFunctions: ITriggerFunctions;
  const fixturesDir = path.join(__dirname, '../../fixtures/webhook-fixtures-anon/purchase-billet-printed');
  ...
  describe('PURCHASE_BILLET_PRINTED Event Processing', () => {
    ...
    test.each(fixtures)('should process $name correctly', async ({ data }) => {
      const webhookData = {
        webhookEvent: 'PURCHASE_BILLET_PRINTED',
    ...
    test('should handle PURCHASE_BILLET_PRINTED in "all-events" mode', async () => {
      const webhookData = {
  ...
  describe('PURCHASE_BILLET_PRINTED Data Validation', () => {
    test('should validate required fields', () => {
      ...
      expect(sampleFixture.event).toBe('PURCHASE_BILLET_PRINTED');
    });
