...
describe('HotmartTrigger - SUBSCRIPTION_CANCELLATION', () => {
  let trigger: HotmartTrigger;
  let triggerFunctions: ITriggerFunctions;
  const fixturesDir = path.join(__dirname, '../../fixtures/webhook-fixtures-anon/subscription-cancellation');
  ...
  describe('SUBSCRIPTION_CANCELLATION Event Processing', () => {
    ...
    test.each(fixtures)('should process $name correctly', async ({ data }) => {
      const webhookData = {
        webhookEvent: 'SUBSCRIPTION_CANCELLATION',
    ...
    test('should handle SUBSCRIPTION_CANCELLATION in "all-events" mode', async () => {
      const webhookData = {
  ...
  describe('SUBSCRIPTION_CANCELLATION Data Validation', () => {
    test('should validate required fields', () => {
      ...
      expect(sampleFixture.event).toBe('SUBSCRIPTION_CANCELLATION');
    });
