...
describe('HotmartTrigger - PURCHASE_OUT_OF_SHOPPING_CART', () => {
  let trigger: HotmartTrigger;
  let triggerFunctions: ITriggerFunctions;
  const fixturesDir = path.join(__dirname, '../../fixtures/webhook-fixtures-anon/purchase-out-of-shopping-cart');
  ...
  describe('PURCHASE_OUT_OF_SHOPPING_CART Event Processing', () => {
    ...
    test.each(fixtures)('should process $name correctly', async ({ data }) => {
      const webhookData = {
        webhookEvent: 'PURCHASE_OUT_OF_SHOPPING_CART',
    ...
    test('should handle PURCHASE_OUT_OF_SHOPPING_CART in "all-events" mode', async () => {
      const webhookData = {
  ...
  describe('PURCHASE_OUT_OF_SHOPPING_CART Data Validation', () => {
    test('should validate required fields', () => {
      ...
      expect(sampleFixture.event).toBe('PURCHASE_OUT_OF_SHOPPING_CART');
    });
