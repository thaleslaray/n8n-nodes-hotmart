...
describe('HotmartTrigger - CLUB_FIRST_ACCESS', () => {
  let trigger: HotmartTrigger;
  let triggerFunctions: ITriggerFunctions;
  const fixturesDir = path.join(__dirname, '../../fixtures/webhook-fixtures-anon/club-first-access');
  ...
  describe('CLUB_FIRST_ACCESS Event Processing', () => {
    ...
    test.each(fixtures)('should process $name correctly', async ({ data }) => {
      const webhookData = {
        webhookEvent: 'CLUB_FIRST_ACCESS',
    ...
    test('should handle CLUB_FIRST_ACCESS in "all-events" mode', async () => {
      const webhookData = {
  ...
  describe('CLUB_FIRST_ACCESS Data Validation', () => {
    test('should validate required fields', () => {
      ...
      expect(sampleFixture.event).toBe('CLUB_FIRST_ACCESS');
    });
