...
describe('HotmartTrigger - PURCHASE_CHARGEBACK', () => {
  let trigger: HotmartTrigger;
  let triggerFunctions: ITriggerFunctions;
  const fixturesDir = path.join(__dirname, '../../fixtures/webhook-fixtures-anon/purchase-chargeback');
  ...
  describe('PURCHASE_CHARGEBACK Event Processing', () => {
    ...
    test.each(fixtures)('should process $name correctly', async ({ data }) => {
      const webhookData = {
        webhookEvent: 'PURCHASE_CHARGEBACK',
    ...
    test('should handle PURCHASE_CHARGEBACK in "all-events" mode', async () => {
      const webhookData = {
  ...
  describe('PURCHASE_CHARGEBACK Data Validation', () => {
    test('should validate required fields', () => {
      ...
      expect(sampleFixture.event).toBe('PURCHASE_CHARGEBACK');
    });
