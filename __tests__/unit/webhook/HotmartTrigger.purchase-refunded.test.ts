...
describe('HotmartTrigger - PURCHASE_REFUNDED', () => {
  let trigger: HotmartTrigger;
  let triggerFunctions: ITriggerFunctions;
  const fixturesDir = path.join(__dirname, '../../fixtures/webhook-fixtures-anon/purchase-refunded');
  ...
  describe('PURCHASE_REFUNDED Event Processing', () => {
    ...
    test.each(fixtures)('should process $name correctly', async ({ data }) => {
      const webhookData = {
        webhookEvent: 'PURCHASE_REFUNDED',
    ...
    test('should handle PURCHASE_REFUNDED in "all-events" mode', async () => {
      const webhookData = {
  ...
  describe('PURCHASE_REFUNDED Data Validation', () => {
    test('should validate required fields', () => {
      ...
      expect(sampleFixture.event).toBe('PURCHASE_REFUNDED');
    });
